...
// ─── Stage config — sla = days before a deal is "stuck" ─────────────────────
const STAGES = [
  {
    key: 'prospect',
    label: 'Prospect',
...
function OppCard({
  ...
  return (
    <motion.div
      ...
      draggable
      onDragStart={(e) => {
        // store opp id in drag transfer for cross-frame safety
        (e as unknown as React.DragEvent).dataTransfer.setData('oppId', opp.id);
        onDragStart(opp);
      }}
      onDragEnd={() => onDragEnd()}
      className={`relative group cursor-grab active:cursor-grabbing select-none transition-opacity duration-150 ${
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ backgroundColor: stuck ? '#ef4444' : stage.accent }}
        />
        ...
        {/* Row 2: title */}
        <p className="text-[11px] font-semibold text-foreground leading-snug pl-3 line-clamp-2 mb-2">
          {opp.title}
        </p>
    ...
    <div className="h-full flex flex-col gap-2.5 min-w-0">
      ...
      <div className="flex items-center justify-between gap-4 shrink-0">
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded-xl bg-sky-500/10 ring-1 ring-sky-500/25">
            <Kanban className="w-4 h-4 text-sky-400" />
          </div>
          <div>
            <h1 className="text-base font-bold text-foreground tracking-tight">Sales Pipeline</h1>
            <p className="text-[11px] text-muted-foreground mt-px">
              {totalDeals} active
              {stuckCount > 0 && <span className="text-red-400"> · {stuckCount} stuck</span>}
            </p>
          </div>
        </div>
        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-sky-600 hover:bg-sky-500 active:scale-95 text-white text-xs font-semibold transition-all shadow-lg shadow-sky-500/20">
          <Plus className="w-3.5 h-3.5" />
          New Opportunity
        </button>
      </div>

      {/* ── KPI strip (compact pill row) ── */}
      {!isLoading && (
      ...
      {isLoading ? (
        ...
        <div
          className="flex-1 overflow-hidden rounded-2xl ring-1 ring-border pipeline-board"
        >
          <div className="flex h-full overflow-x-auto">
            {STAGES.map((stage, idx) => {
              ...
              const opps: Opportunity[] = [...(col?.opportunities ?? []) as Opportunity[]].sort((a, b) => {
                // Primary: age descending (oldest = most urgent = top)
                const ageDiff = agingDays(b.createdAt) - agingDays(a.createdAt);
                if (ageDiff !== 0) return ageDiff;
                // Secondary: explicit priority (high → medium → low)
              ...
              return (
                <div
                  key={stage.key}
                  className={`flex flex-col h-full flex-1 min-w-[220px] max-w-[320px] ${
            ...
            <div
              className="flex flex-col h-full shrink-0 border-l border-border/40"
              style={{ width: 180 }}
            >
